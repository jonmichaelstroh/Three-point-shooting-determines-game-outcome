...
For example, in the first round of the NCAA tournament, 1 seed Baylor beat 16 seed Hartford 79 to 55.  According to the model, the odds that Baylor would win the game given its game statistics are expected to be  `r round(exp(3.987), 3)` ($exp^{-3.263 + 0.095(47.6) + 0.090(33.3) - 0.214(10) + 0.277(15) - 0.229(16) + 0.138(10)}$).  This indicates that given its game statistics, Baylor is expected to win `r round(exp(3.987), 3)` games for every one loss.  This value may seem extreme but Baylor's game stats were excellent, especially their total number of steals.  Similarly, given the same formula, according to the model, the odds that Hartford would win the game given its game statistics are expected to be `r round(exp(-2.175), 3)`.  Thus, Hartford would be expected to win `r round(exp(3.987), 3)` games for every loss.  The odds `r round(exp(3.987), 3)` means for every 1 win the team is expected to lose roughly 9 games.
